		...
		<NcAppContent-->
			...
			<div v-else>
				...
				<div v-else id="emptycontent">
					<img src="../../img/crowsito.png" width="120px">
					<h2>
...
export default {
	...
	async mounted() {
		const user = await axios.get('/ocs/v2.php/cloud/user');
		this.gethistorial(user.data.ocs.data.id)
	},

	methods: {
		async gethistorial(id_user){
				
			//user.data.ocs.data.id <- get username
			try {
				let response = await axios.get(generateUrl('apps/ahorrosgossler/getHistorial/' + id_user));
